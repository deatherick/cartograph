...
**Status**: MVP shipped (Phase 2), plus Go (Phase 3a), C# (Phase 3b), and Python (Phase 3c)
support, a plug-and-play language architecture (Phase 3a′), true per-file incremental indexing
(Phase 3d), impact analysis (Phase 4), a similarity/duplicate engine V0 (Phase 5), and a
React-based web UI (Phase 6) — TypeScript, Go, C#, and Python extraction/resolution, the Context
Compiler, an MCP server, `ctxd` (indexes once, then watches and incrementally re-indexes on
change), `ctx impact`/git-diff blast radius, `ctx duplicates`/`similar`/`decide`, and a browser UI
(an integrated Overview with a searchable/filterable entity table, a navigable graph + tree view,
...
- **A C compiler** (`cc`/`clang`/`gcc`) — every language extractor uses tree-sitter via cgo
...
Now point it at a real repo instead — TypeScript, Go, C#, Python, or a mix in the same repo, same
...
- **TypeScript/JavaScript, Go, C#, and Python — every originally-planned language is built.**
  ...
  ([ADR-0023](docs/adr/0023-csharp-extractor.md)). Python follows the same standard: `self`'s type
  resolves deterministically (it names its own enclosing class), but a field only gets a known type
  from `self.x = SomeClass(...)`, never from a bare `self.x = some_parameter`, and absolute imports
  resolve against the repo root with no `src/`-layout guessing
  ([ADR-0024](docs/adr/0024-python-extractor.md)).
